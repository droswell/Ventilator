...
def connect():
    ...
    serial_object = serial.Serial('/dev/ttyACM0', 9600)

    print("Entered the connect function")
    btnconnect['state'] = DISABLED
    btndisconnect['state'] = NORMAL
...
if __name__ == "__main__":
    ...
    port_entry.insert(0,'/dev/ttyACM0')
    port_entry.place(x=200, y=365)

    # button
    button1 = Button(text="Send", command=send, width=6).place(x=15, y=250)
    btnconnect = Button(text="Connect", command=connect).place(x=15, y=360)
    btndisconnect = Button(text="Disconnect", command=disconnect).place(x=300, y=360)
